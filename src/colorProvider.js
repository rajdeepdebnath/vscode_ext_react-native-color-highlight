import {
  ...
  // #rrggbb or #rrggbbaa
  const colorMatch =
    /#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{1,2})?/;
  // rgb or rgba
  ...
  export const colorProvider = {
    provideColorPresentations(color) {
      ...
      // formatHex doesn't support alpha so we need to add it on for now until https://github.com/d3/d3-color/issues/78 is done
      // https://stackoverflow.com/questions/15852122/hex-transparency-in-colors
      const alphaForHex =
        color.alpha !== 1
          ? (((color.alpha * 100 * 255) / 100) | 0).toString(16)
          : "";
      return [
    ...
    provideDocumentColors(document) {
      if (!document.getText().includes(onlyReactNativeStyle)){
        return;
      }
      ...
      for (let i = 0; i < document.lineCount; i++) {
        ...
        let match;
        if ((match = totalRegex.exec(element.text))) {
